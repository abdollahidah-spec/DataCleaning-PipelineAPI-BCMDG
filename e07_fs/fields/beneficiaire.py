...
==================================
Normalisation du champ Beneficiaire pour E07_FS — porté depuis
DataCleaning-PipelineField-BCMDG/beneficiaire/normalize_beneficiaire.py
(fonction `treating_beneficiaire_web`, seule variante utilisée par E07).

Pour un crédit documentaire IMPORT (E07_FS), le bénéficiaire est une entité
...
que NomDonneurOrdre (e07_fs/fields/_entity_matching.py) — les deux champs
...
sont écrites dans validated_classif_beneficiaire_e07_fs.json.
...
def treating_beneficiaire(
    ...
    df["Beneficiaire_Normalisé"] = df[corr_col].map(result_map_iso)
    df["Beneficiaire_method"]    = df[corr_col].map(result_map_mth)
