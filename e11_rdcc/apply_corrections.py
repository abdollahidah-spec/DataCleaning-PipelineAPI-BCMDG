"""
e11_rdcc/apply_corrections.py
================================
Applique les corrections manuelles saisies dans l'onglet "Instructions" d'un
fichier {api_id}_mapping_outliers_{ts}.xlsx vers les caches warm-start des
champs concernés. La colonne "Champ" route chaque ligne vers le bon champ —
un seul onglet couvre maintenant tous les champs de l'API (contrairement à
l'ancien repo, qui avait un classeur par champ et donc pas besoin de ce
discriminant).
...
Usage (depuis la racine du repo) :
    python -m e11_rdcc.apply_corrections --file "e11_rdcc/outputs/E11_RDCC_mapping_outliers_20260727_101500.xlsx"
"""
...
from e11_rdcc.pipeline import build_e11_field_processors

_REQUIRED_COLS = {"Champ", "Input", "Label_Attendu"}
_RE_SPACES = re.compile(r"\s+")
_VALID_API_IDS = ("E11_RDCC",)
...
def apply_corrections(file_path: str | Path, api_id: str, config_path: str | Path) -> dict:
    ...
    processors = {p.field_name: p for p in build_e11_field_processors(cfg)}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file", required=True, help="Fichier {api_id}_mapping_outliers_{ts}.xlsx")
    parser.add_argument("--api-id", default=None, choices=_VALID_API_IDS)
    parser.add_argument("--config", default="e11_rdcc/config/E11_RDCC.yaml")
