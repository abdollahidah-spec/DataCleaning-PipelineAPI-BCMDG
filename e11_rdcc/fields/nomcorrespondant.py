...
pour E11_RDCC — porté depuis DataCleaning-PipelineField-BCMDG/nomcorrespondant/
normalize_nomcorrespondant.py, sans changement de logique métier.

Champ propre à l'API E11 (voir décision H du plan) : référentiel et cache
warm-start vivent dans e11_rdcc/referentiel/, pas dans un module top-level
partagé entre APIs — une future API (E07_FS, E08_OCD...) aura sa propre copie,
libre de diverger.
...
CASCADE :
  1. Warm-start (validated_classif_nomcorrespondant_e11_rdcc.json) → WARM
  2. Référentiel exact (déjà validé BCM)                           → MAP
  3. Fallback API Claude                                           → CLAUDE si résolu, sinon OUTLIER

PERSISTANCE DU CACHE CLAUDE :
  L'API Claude étant payante, toute modalité qu'elle a dû résoudre (nom trouvé
  ou OUTLIER) est automatiquement écrite dans le cache warm-start. Dès
  l'exécution suivante, cette modalité est résolue en WARM et n'est plus jamais
  renvoyée à Claude.

RÈGLE NA :
  NomCorrespondant == 'NA'  ET  Ref == 'NA'   → 'NA'
  NomCorrespondant == 'NA'  ET  Ref != 'NA'   → OUTLIER
...
def treating_nomcorrespondant(
    ...
    if ref is None:
        ref = load_nomcorrespondant_referentiel(_REFERENTIEL_DIR / "nomcorrespondant_referentiel_E11.json")
...
def build_full_classification_nomcorrespondant(ref: dict, api_id: str, col_in: str, col_out: str) -> pd.DataFrame:
    ...
    lignes du run en cours — voir build_full_classification_devise() pour le
    raisonnement complet (mode incrémental). Le cache (Claude passé + corrections
    manuelles) prime sur le référentiel statique.
    """
    combined = dict(ref)
