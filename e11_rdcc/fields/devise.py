"""
e11_rdcc/fields/devise.py
============================
Normalisation du champ Devise pour E11_RDCC — porté depuis
DataCleaning-PipelineField-BCMDG/devise/normalize_devise.py, sans changement
de logique métier.

Champ propre à l'API E11 (voir décision H du plan). Différence vs l'ancien
repo : le cache warm-start devient E11-scopé (validated_classif_devise_e11_rdcc.json)
au lieu d'être global à toutes les APIs — corrige un partage de cache non
souhaité entre APIs distinctes. `save_warm_start_devise()` est un ajout neuf
(l'ancien repo n'avait qu'un cache Devise en lecture seule, jamais écrit par
...
def load_warm_start_devise(api_id: str = "E11_RDCC") -> dict:
...
def save_warm_start_devise(api_id: str, new_entries: dict, verbose: bool = False) -> None:
    ...
    dans le cache warm-start Devise de cette API. Gap-fill : n'existait pas dans
    l'ancien repo (cache Devise en lecture seule) — mirroring exact de
    save_warm_start_nomcorrespondant() (mêmes clés top-level, tri, indentation).
    """
    if not new_entries:
        return
    path = _warm_start_path(api_id)
