"""
Rapport PDF E10 (Markdown avant conversion) : même gabarit que les autres APIs,
avec les libellés du ticket E10 et le rattachement de chaque règle de validation
à son champ. Produit (hors périmètre) et SourceDevise (absente de la table)
n'apparaissent pas.
"""
...
def _quality_report() -> QualityReport:
    return QualityReport(
        api_id="E10_FE", mode="incremental",
...
def _results() -> list:
    ...
    swift_result = swift.process(
        pd.DataFrame({"RefBanque": ["B1", "B2"], "TypeSwfit": ["XYZ", "ABC"]}), api_id="E10_FE"
    )
...
def test_quality_report_generique():
    ...
    assert "Rapport de qualité des traitements" in md and "E10_FE" in md
