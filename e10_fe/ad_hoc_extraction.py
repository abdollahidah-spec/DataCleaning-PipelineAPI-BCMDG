"""
e10_fe/ad_hoc_extraction.py
=============================
Extraction ad hoc E10_FE, INDÉPENDANTE du run automatisé — même outil que
...
Usage (depuis la racine du repo) :
    ...
    python -m e10_fe.ad_hoc_extraction --config e10_fe/config/E10_FE.yaml \\
        --input tests/fixtures/e10_fe_sample.csv
"""
...
from e10_fe.fields.transactions import TransactionsProcessor
...
def run_ad_hoc_extraction(
    ...
    pipeline = E10Pipeline(cfg, config_source=config_path)
    ...
    out_path = Path(output_path) if output_path else Path(f"e10_fe/outputs/ad_hoc_extraction_{ts}.csv")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Extraction ad hoc E10_FE (requête SQL personnalisée, table, ou fichier local)")
    parser.add_argument("--config", default="e10_fe/config/E10_FE.yaml", help="Chemin du YAML E10_FE")
    ...
    parser.add_argument("--output", default=None,
                         help="Chemin du CSV de sortie (défaut : e10_fe/outputs/ad_hoc_extraction_{ts}.csv)")
