...
====================================
Normalisation du champ NatureEconomique pour E10_FE (Flux Entrants) — portée
depuis l'ancien repo field-based (nature_economique/normalize_nature_economique.py,
flux FE) : mêmes données métier (référentiel catégorie -> labels, mapping direct,
...
# ══════════════════════════════════════════════════════════════════════════════
# DONNÉES MÉTIER — reprises telles quelles de l'ancien repo (flux FE)
# ══════════════════════════════════════════════════════════════════════════════
...
_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "(transferts reçus — flux entrants).\n"
...
def load_nature_economique_referentiel(path) -> NatureEconomiqueReferentiel:
    ...
    return NatureEconomiqueReferentiel(
        ...
        mapping_direct=dict(_MAPPING_DIRECT_FE),
        non_classe=set(_NON_CLASSE_DIRECT_FE),
    )
...
def load_warm_start_nature_economique(api_id: str = "E10_FE") -> dict:
    path = _warm_start_path(api_id)
    if not path.exists():
        return {}
    classif = json.load(open(path, encoding="utf-8")).get("classif", {})
    # « NON CLASSE » : ancienne catégorie du flux FE, remplacée par OUTLIER dans
    # l'ancien repo — encore présente dans le cache hérité, lue comme OUTLIER.
    valeurs = {k: str(v).strip() for k, v in classif.items()}
    return {k: ("OUTLIER" if v == "NON CLASSE" else v) for k, v in valeurs.items()}
...
def treating_nature_economique(
    ...
    df = df.copy()
    if ref is None:
        ref = load_nature_economique_referentiel(_REFERENTIEL_DIR / "nature_economique_referentiel_E10.json")
    cfg = cfg or {}
