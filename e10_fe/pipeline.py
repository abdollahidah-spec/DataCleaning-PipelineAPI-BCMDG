...
e10_fe/pipeline.py
====================
Pipeline consolidée E10_FE (Flux Entrants) : 7 champs catégoriels — TypeSwift,
...
from e10_fe.fields.beneficiaire import build_beneficiaire_processor
from e10_fe.fields.devise import build_devise_processor
from e10_fe.fields.mode_reglement import build_mode_reglement_processor
from e10_fe.fields.nature_economique import build_nature_economique_processor
from e10_fe.fields.nomdonneurordre import build_nomdonneurordre_processor
from e10_fe.fields.pays import build_pays_processor
from e10_fe.fields.transactions import build_transactions_processor
from e10_fe.fields.typeswift import build_typeswift_processor
...
def build_e10_field_processors(cfg: dict) -> list:
    """
    Construit la liste des FieldProcessor à partir de cfg["fields"].
    `type: categorical` -> le `name` du champ sélectionne le module e10_fe.fields.* ;
    ...
    for field_cfg in cfg["fields"]:
        ...
        if ftype == "categorical":
            ...
            if builder is None:
                raise ValueError(f"Champ catégoriel inconnu pour E10_FE : {name!r}")
            ...
            raise ValueError(f"Type de champ inconnu pour E10_FE : {ftype!r}")
    return processors


class E10Pipeline(BaseApiPipeline):
    def _build_field_processors(self, cfg: dict) -> list[FieldProcessor]:
        return build_e10_field_processors(cfg)
    ...
    def build_reports_markdown(self, results: list, quality) -> str:
        ...
        from e10_fe.reports import build_outliers_report_markdown, build_quality_report_markdown
